  ...
  if (message.content.startsWith(prefix + "anti")) {
    ...
    }
    {
      let black = new Discord.MessageEmbed()
        ...
        .setFooter("Protection Yarz system")
        .setColor("#00F0FF");
...
.setLabel('Github')
.setStyle('url')
.setEmoji('939812272203763773')
.setURL('https://github.com/Uunkn0wnN');
...
//discord https://discord.gg/UFTXkxRrWT//code by Uunkn0wnN
...
Pertama-tama, halo
semua, seperti setiap server, memiliki aturan kami sendiri
1. Anda tidak boleh mengatakan sesuatu yang tidak pantas atau bersumpah karena kami akan segera melarang Anda
2. Mereka seharusnya tidak menggunakan suara publik untuk Anda
3. Iklan dilarang dengan cara apapun kecuali pertukaran
4. Anda tidak boleh tidak menghormati anggota staf individu dengan cara apa pun
5. Dilarang mengganggu pemain dan tamu
6. Dilarang membicarakan politik dengan cara apapun
7. Kewarganegaraan dilarang
8. Dilarang menghina seseorang dan langsung dilarang
9. Dilarang mengulang pesan atau spamming
dengan hormat....!
   `)
   message.channel.send(blackjack)
...
client.on("message", async message => {
  if (message.content.startsWith(prefix + "settopic")) {
    ...
    let topic = message.content
      ...
      .slice(1)
      .join(" ");
    if (!topic) return message.channel.send("**Topic**");
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "lock")) {
    let blackjack = "created by Yarz Protection";
...
.addField("``OWNER``", `[ <@820532177853743105> ]`, true)
.addField("``ADMIN``", `[ <@820532177853743105> ]`, true)
...
discord https://discord.gg/UFTXkxRrWT`)).listen(process.env.PORT || 8080)
...
client.on("roleUpdate", (oldRole, newRole) => {
  ...
  oldRole.guild.fetchAuditLogs().then(logs => {
    ...
    if (oldRole.hexColor !== newRole.hexColor) {
      if (oldRole.hexColor === "#00F0FF") {
        var oldColor = "`Default`";
      } else {
        var oldColor = oldRole.hexColor;
      }
      if (newRole.hexColor === "#00F0FF") {
